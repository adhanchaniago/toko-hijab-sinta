...
class Item extends CI_Controller {
	...
	public function ajax_list()
   {
      ...
      foreach ($list as $i) {
         ...
         $row[] = '<a href="'.site_url('item/detail/'.$i->id_item).'" class="btn btn-success btn-xs"><i class="fa fa-search-plus"></i></a>
				...
				<a href="'.site_url('item/del_item/'.$i->id_item).'" class="btn btn-danger btn-xs" onclick="return confirm(\'Yakin ingin menghapus data ini?\')"><i class="fa fa-trash"></i></a>';
   ...
   public function add_item()
   {
      ...
      if ($this->input->post('submit', TRUE) == 'Submit') {
         ...
         if ($this->form_validation->run() == TRUE)
         {
				...
				if ($this->upload->do_upload('foto'))
				{
		         ...
		         $items = array (
		        	...
		        	$id_item = $this->items->insert_last('tbl_item', $items);
					...
					for ($i=0; $i < $len; $i++) {
				      ...
				      $_FILES[$foto]['tmp_name'] = $_FILES['gb']['tmp_name'][$i];
						...
						if ($this->upload->do_upload($foto))
						{
							...
							//insert data img
							$this->items->insert('tbl_img', $data);
						}
			      }
		...
		$data['kat'] 		= $this->items->get_all('tbl_kategori');
	...
	public function detail()
	{
		...
		$item = $this->items->get_where('tbl_item', array('id_item' => $id_item));
		...
		$table = "tbl_detail_kategori rk
						JOIN tbl_kategori k ON (rk.id_kategori = k.id_kategori)";
		$data['kategori'] = $this->items->get_where($table, ['rk.id_item' => $id_item]);
		//ambil data img berdasarkan id_item
		$data['img'] = $this->items->get_where('tbl_img', ['id_item' => $id_item]);
	...
	public function update_item()
   {
      ...
      if ($this->input->post('submit', TRUE) == 'Submit') {
         ...
         if ($this->form_validation->run() == TRUE)
         {
				...
				if ($this->upload->do_upload('foto'))
				{
		         ...
		         $this->items->update('tbl_item', $items, array('id_item' => $id_item));
				} else {
					$this->items->update('tbl_item', $items, array('id_item' => $id_item));
				}

				$this->items->delete('tbl_detail_kategori', ['id_item' => $id_item]);
				$this->kategori($id_item, $this->input->post('kategori', TRUE));
		...
		$gb = $this->items->get_where('tbl_img', ['id_item' => $id_item]);
		//cek data img
		if ($gb->num_rows() != 0)
		{
			$data['gb'] = $gb;
	...
	private function kategori($id_item, $kategori)
	{
		...
		for ($i = 0; $i  < $len; $i ++) {
			...
			$cek = $this->items->get_where('tbl_kategori', ['url' => $url]);
			...
			if ($cek->num_rows() > 0) {
				...
				$id = $this->items->insert_last('tbl_kategori', $data);
			}

			$cek2 = $this->items->get_where('tbl_detail_kategori', ['id_item' => $id_item, 'id_kategori' => $id]);

			if ($cek2->num_rows() < 1) {
				$this->items->insert('tbl_detail_kategori', ['id_item' => $id_item, 'id_kategori' => $id]);
			}
		}
	}
	...
	public function del_img()
	{
		...
		$this->items->delete('tbl_img', ['img' => $this->uri->segment(3)]);
	...
	public function update_img()
	{
		...
		if ($this->input->post('submit', TRUE) == 'Submit')
		{
			...
			if ($this->upload->do_upload('img'))
			{
				...
				$this->items->update('tbl_img', ['img' => $gbr['file_name']], ['img' => $this->uri->segment(3)]);
	...
	public function del_item()
	{
		...
		$this->items->delete(['tbl_item'], ['id_item' => $this->uri->segment(3)]);
