	...
	public function index()
	{
		...
		if ($this->input->post('submit', TRUE) == 'Submit')
      {
         ...
         $this->form_validation->set_rules('prov', 'Provinsi', 'required');
			...
			$this->form_validation->set_rules('kd_pos', 'Kode Pos', 'required|numeric|max_length[5]');
         ...
         if ($this->form_validation->run() == TRUE)
         {
				if ($this->session->userdata('user_id'))
				{
            	$get = $this->app->get_where('tbl_user', ['id_user' => $this->session->userdata('user_id')]);
					$user = $get->row();

					$nama_pemesan = $user->nama_user;
					$email = $user->email;
				} else {
            ...
            $this->email->message(
               'Terima Kasih telah melakukan pemesanan di toko kami, selanjutnya silahkan anda mentransfer uang senilai <b>Rp. '.number_format($total, 0, ',', '.').',-</b> ke no. rekening <b>'.$profil->rekening.'</b> segera lakukan pembayaran dan upload bukti pembayaran sebelum tanggal '.$bts_bayar.' agar pesanan anda bisa kami proses. Detail pembayaran sebagai berikut :<br/><br/>
					Id Order : '.$id_order.' ('.$tgl_pesan.')<br/><br/>
            ...
            if ($this->email->send())
            {
               $data = array(
					...
					if ($this->app->insert('tbl_order', $data)) {
						...
						foreach ($this->cart->contents() as $key) {
							...
							$this->app->insert('tbl_detail_order', $detail);
						}
		            ...
		            $this->email->message(
		               'Hai admin,<br /><br />Ada pesanan baru dari '.$profil->nama_toko.' dengan ID pesanan '.$id_order.' pada tanggal '.date('d M Y', strtotime($tgl_pesan)).'. Silahkan login untuk melihat detail pesanan secara lengkap.'
		            );
		...
		$key['key']  = $this->app->get_where('tbl_profil_toko', ['id_profil' => 1]);
   ...
   public function city()
   {
		if (!$this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() == TRUE)
			{
		      $prov = explode(",", $this->encryption->decrypt($this->input->post('prov', TRUE)));
				$key  = $this->app->get_where('tbl_profil_toko', ['id_profil' => 1])->row();
		      $curl = curl_init();
	...
	public function getcost()
	{
		if (!$this->input->is_ajax_request()) {
			...
			if ($this->form_validation->run() == TRUE)
			{
				$api  = $this->app->get_where('tbl_profil_toko', ['id_profil' => 1])->row();
				$asal = $api->asal;
